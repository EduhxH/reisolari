...
async def list_listings(
    ...
    if owner_id:
        query["owner_id"] = require_object_id(owner_id, "owner_id")
...
async def list_my_listings(current_user: dict = Depends(get_current_user)):
    db = get_database()
    cursor = db.listings.find({"owner_id": current_user["_id"]}).sort("created_at", -1)
    return [serialize_listing(doc) async for doc in cursor]
